...
### 🔍 Explore Community
Discover stories from other photographers at `racconto.app/explore`. Chronological feed (no algorithm, no likes) of portfolios that artists opt-in to share. Filter by camera type (Film / Digital / Mobile / Mixed) and genre tags. Search by username or portfolio title. Follow your favorite photographers — follower count stays private, only the artist sees their own.
...
### 🔍 둘러보기 커뮤니티
`racconto.app/explore` 에서 다른 사진가들의 이야기를 만나 보세요. 알고리즘 없이, 좋아요 없이 — 작가가 직접 공개한 포트폴리오만 시간순으로 보여줍니다. 카메라 종류(필름/디지털/모바일/혼합)와 장르 태그로 필터, 사진가 이름과 포트폴리오 제목 검색 가능. 마음에 든 작가를 팔로우하세요 — 팔로워 수는 본인만 볼 수 있어 부담 없는 연결이 가능합니다.
